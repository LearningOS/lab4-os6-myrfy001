...
pub fn hardlink(src: &str, dst: &str) -> Option<()>{
    if ROOT_INODE.hardlink(src, dst).is_some() {
        Some(())
    } else {
        None
    }
...
impl File for OSInode {
    ...
    fn fstat(&self) -> Option<Stat> {
        let mut inner = self.inner.exclusive_access();
        let s = inner.inode.get_fstat();
    
        let mode = match s.mode {
            EfsStatMode::NULL => StatMode::NULL,
            EfsStatMode::DIR => StatMode::DIR,
            EfsStatMode::FILE => StatMode::FILE,
        };
